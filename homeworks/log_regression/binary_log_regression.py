...
class BinaryLogReg:
    @problem.tag("hw2-A", start_line=3) # Done
    def __init__(self, _lambda: float = 1e-3):
        """Initializes the Binary Log Regression model. NOTE: Please DO NOT change `self.weight` and `self.bias`
        values, since it may break testing and lead to lost points!
        ...
        self.bias: float = None # type: ignore

    def sigmoid(self, z): # Done
        return 1 / (1 + np.exp(-z))

    @problem.tag("hw2-A") # Done
    ...
    def predict(self, X: np.ndarray) -> np.ndarray:
        ...
        """
        predictions = self.sigmoid(np.dot(X, self.weight.T) + self.bias)
        predictions = np.array(predictions)

        prediction_class = []
        if X.shape != (X.shape[0],):
            prediction_class = [1 if i > 0.5 else -1 for i in predictions]
        else:
            prediction_class.append(1 if predictions >= 0.5 else -1)
    ...
    def misclassification_error(self, X: np.ndarray, y: np.ndarray) -> float:
        ...
        Returns: float: percentage of times prediction did not match target, given an observation (i.e.
    ...
    def step(self, X: np.ndarray, y: np.ndarray, learning_rate: float = 1e-4):
        ...
        db = self.gradient_J_weight(X,y)

        self.weight -= learning_rate * dw
        self.bias -= learning_rate * db

    @problem.tag("hw2-A", start_line=7) # Done
    def train(self, X_train: np.ndarray, y_train: np.ndarray, X_test: np.ndarray, y_test: np.ndarray,
              learning_rate: float = 1e-2, epochs: int = 30, batch_size: int = 100) -> Dict[str, List[float]]:
        ...
        NOTE: This function due to complexity and number of possible implementations will not be publicly unit
        tested. However, we might still test it using gradescope, and you will be graded based on the plots that are
        ...
        for i in range(epochs):
            ...
            print(f'finished epoch {i}, train loss: {result["train_losses"][-1]}, test loss: {result["test_losses"][-1]}')
...
if __name__ == "__main__":
    ...
    history = model.train(x_train, y_train, x_test, y_test, epochs=100, learning_rate=1e-2, batch_size=1)
